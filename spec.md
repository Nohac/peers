...
- Current-side added and context rows should mirror syntax/highlight state from hidden real file buffers so the review buffer follows the user's Neovim theme and language setup. This mirroring must be viewport-scoped for performance on large reviews. The Lua mirror uses Neovim inspection APIs such as `vim.inspect_pos` for syntax/Tree-sitter groups. For LSP semantic tokens, hidden Peers source-helper buffers should request `textDocument/semanticTokens/full` directly from attached clients, parse and line-index the returned token ranges, and synthesize the same `@lsp.type.*`, `@lsp.mod.*`, and `@lsp.typemod.*` highlight stacks that Neovim applies in visible source windows; `vim.lsp.semantic_tokens.get_at_pos()` and inspected semantic extmarks can remain fallbacks. Peers resolves those highlight stacks into concrete foreground/style attributes per range, caches Peers-owned highlight groups by those attributes, and applies those concrete groups in the review buffer. Source background colors should not be copied because diff rows have Peers-owned backgrounds. Deleted/base-side rows may start with structural diff coloring only.
