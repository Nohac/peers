...
- Review-owned shortcut actions should work from both the review buffer and sidebar when the selected row has enough metadata: `c` opens the composer or replies to the selected thread/comment, visual `c` opens a range comment for selected diff lines, `A` asks the configured agent to do a full review of all currently open threads, `R` asks the configured agent to respond to and resolve the selected thread with code changes, `C` asks the configured agent to comment on the selected thread without code changes, `D` deletes the selected editable comment with confirmation, `r` toggles the selected thread between resolved and reopened, and `x` toggles collapsed/expanded thread state. Sidebar actions proxy through the owning review buffer rather than running a separate LSP client.
